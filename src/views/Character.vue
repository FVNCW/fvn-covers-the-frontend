            ...
            <n-card title="角色详情" size="small" style="max-width: 720px; margin: 0 auto">
                <n-descriptions label-placement="left" :column="1" bordered>
                    ...
                    <n-descriptions-item label="种类">
                        {{ Array.isArray(character.specy) ? "混血种" : "纯血种" }}（{{
                            specyNames
                        }}）
                    </n-descriptions-item>
                    ...
                    <n-descriptions-item label="来源作品">
                        <template v-if="character.information.sources.length">
                            <n-tag
                                v-for="s in character.information.sources"
                                :key="s"
                                size="small"
                                style="margin-right: 6px"
                            >
                    ...
                    <n-descriptions-item
                        v-for="(composed, key) in character.color.other"
                        :key="'c-' + key"
                        :label="`其他颜色（${key}）`"
                    >
                    ...
                    <n-descriptions-item
                        v-for="(ids, key) in character.relationShips.other"
                        :key="'r-' + key"
                        :label="`关系（${key}）`"
                    >
                        {{ relationNames(ids) || "无" }}
                    ...
                    <n-descriptions-item label="立绘">
                        <template v-if="currentPortrait">
                            <div class="carousel">
                                ...
                                <div class="stage">
                                    <img
                                        :src="imgUrl(currentPortrait.objectId)"
                                        :alt="currentPortrait.displayName"
                                    />
                                    <div class="caption">
                                        {{ currentPortrait.displayName }}
                                        <n-tag
                                            v-for="t in currentPortrait.tags"
                                            :key="t"
                                            size="tiny"
