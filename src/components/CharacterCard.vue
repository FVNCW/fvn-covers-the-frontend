...
const extraRelationCount = computed(
	() => Math.max(0, relationCharacters.value.length - 3),
);
...
<template>
	<div class="card" v-if="character && !loading">
		<!-- 顶部插图区域 -->
		<div class="cover"
			:style="{ backgroundImage: coverIllustrationId ? `url(/api/illustration/download/${coverIllustrationId})` : 'none' }">
		...
		<div class="content">
				...
				<div class="colors">
					<div class="color-item" v-if="char.color.fur.length">
						<div class="color-swatch"
							:style="{ backgroundColor: rgbToHex(char.color.fur[0]!.r, char.color.fur[0]!.g, char.color.fur[0]!.b) }">
						</div>
						<span>{{ colorLabel("fur") }}</span>
					</div>
					<div class="color-item" v-if="char.color.eye.length">
						<div class="color-swatch"
							:style="{ backgroundColor: rgbToHex(char.color.eye[0]!.r, char.color.eye[0]!.g, char.color.eye[0]!.b) }">
						</div>
			...
			<!-- 出处 -->
			<div class="section" v-if="char.information?.sources && char.information.sources.length">
